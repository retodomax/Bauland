...
plot_leaflet <- function(mydata = data,
  ...
  if (plot_empty) {
    ...
    data_map$value <- extract_values(mydata = mydata,
                                     ...
                                     myyear = myyear)
    myleaf %>% 
      addPolygons(data = data_map, layerId = ~bfs_nr, color = "gray",
                  opacity = 1, weight = 0.5, smoothFactor = 0.5,
                  fillOpacity = 0.7,
                  highlightOptions = highlightOptions(color = "gray", weight = 1.5),
                  fillColor = ~pal(value),
                  label = ~paste0(bfs_name, ": ", formatC(value))) %>% 
      addLegend(data = data_map, pal = pal, values = ~value, opacity = 1, title = "Legende") %>%
      addPolygons(data = ch_see, color = rgb(77,118,205, maxColorValue = 255),
                  opacity = 1, weight = 0.7, smoothFactor = 0.5) %>% 
      {if(tiles) addTiles(.) else .}
  }
...
server <- function(input, output, session){
  ...
  output$trend <- renderPlot({
    ...
    if(!is_empty(input$reso) &
       !is_empty(input$year) &
       !is_empty(input$mymap_shape_click)){
      
      
      plotdata <- data %>% 
        filter(target == input$target,
               reso == input$reso,
               nr == input$mymap_shape_click[1]) %>% 
        arrange(year)
      
      plot(value ~ year, data = plotdata, type = "p",
           xlab = "Jahr", ylab = "")
